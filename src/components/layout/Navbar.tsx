    ...
    <header className="fixed inset-x-0 top-3 z-50 flex justify-center px-3 sm:top-4 sm:px-5 lg:top-6 lg:px-6">
      <nav aria-label="Primary navigation" className="w-full max-w-[1650px] rounded-[24px] border border-white/50 bg-white/90 shadow-[0_20px_60px_rgba(0,0,0,.10)] backdrop-blur-2xl transition-all duration-500 lg:rounded-[30px]">
        <div className="flex h-16 items-center justify-between px-4 sm:h-[72px] sm:px-6 lg:h-[78px] lg:px-10">
          <a href="/#home" aria-label="KP Spec Solutions home" className="transition duration-300 hover:scale-[1.03]">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <a href="/#presentation" className="hidden min-h-11 items-center rounded-full bg-[#8F9F7A] px-5 text-[11px] font-semibold uppercase tracking-[0.16em] text-white shadow-lg transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#798868] hover:shadow-xl sm:inline-flex lg:px-7 xl:px-8">Request a Presentation</a>
            <button type="button" className="inline-flex size-11 items-center justify-center rounded-full text-[#2B2B2B] transition hover:bg-[#F0EEE8] xl:hidden" aria-label={menuOpen ? "Close navigation menu" : "Open navigation menu"} aria-expanded={menuOpen} aria-controls="mobile-navigation" onClick={() => setMenuOpen((open) => !open)}>
        ...
        {menuOpen && (
          <div id="mobile-navigation" className="border-t border-[#E6E1D8] px-5 pb-6 pt-3 xl:hidden">
            ...
            <a href="/#presentation" onClick={() => setMenuOpen(false)} className="mt-5 inline-flex min-h-12 w-full items-center justify-center rounded-full bg-[#8F9F7A] px-5 text-center text-xs font-semibold uppercase tracking-[0.16em] text-white sm:hidden">Request a Presentation</a>
          </div>
        )}
      </nav>
    </header>
